...
while (iter < iter_max)        
    ...
    grps = SpectralClusteringstr(CKSym, nbcluster, grps); 
    
    if (nbcluster >10)
        missrate = 0; % it is an approximate way to calculate ACC.
    else
        missrate = 0;%Misclassification(grps, idx'); % it sometimes causes out of memory when nbcluster >10 
    end
